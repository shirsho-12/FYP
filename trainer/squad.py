...
from hg_parser import ConstituencyParser, ConstituencyNode
...
def squad_convert_example_to_features(
    ...
    for span in spans:
        ...
        for index, token in enumerate(span_doc_tokens):
            ...
            token_node_mapping[index] = (
                len(truncated_query) + sequence_added_tokens + index
            )
        ...
        span["context"] = tokenizer.decode(
            span["input_ids"][sep_index + 1 :], skip_special_tokens=True
        )
...
def squad_convert_examples_to_features(
    ...
    for feature in tqdm(features, desc="constituency graph construction"):
        ...
        context = tokenizer.decode(
            feature.input_ids, skip_special_tokens=True
            ...
            for constituent in constituents:
                ...
                for leaf in leaves:
                    ...
                    while current_token_text != leaf:
                        current_token_text += (
                            toki[last_token_id]
                            if not toki[last_token_id].startswith("##")
                            else toki[last_token_id][2:]
                        )
                        current_token_ids += [last_token_id]
                        last_token_id += 1
                    leaf_node_mapping[leaf_id] = current_token_ids
        ...
        try:
            ...
            def iterate_tree(root, R_cc, R_ct, labels):
                ...
                if root.is_preterminal():
                    ...
                    is_answer = False
                    if (
                        tids[0] == start_position - 2
                        and tids[-1] == end_position - 2
                        and end_position != 0
                    ):
                        is_answer = True
                    ...
                    for child in root.children:
                        child_node, R_cc, R_ct, labe = iterate_tree(
                            child, R_cc, R_ct, labels
                        )
                        labels1.update(labe)
            ...
            for constituent in constituents:
                ...
                try:
                    root, R_cc, R_ct, lab = iterate_tree(
                        constituent.constituency, list(), list(), dict()
                    )
                    r_t.extend(R_ct)
                    r_c.extend(R_cc)
                    labels.update(lab)
            ...
            c_edge_index = {
                ...
                ("constituent", "connect", "constituent"): torch.tensor(
                    r_c
                ).contiguous(),
            }
            ...
            data["token"].x = torch.reshape(
                torch.tensor(feature.input_ids), (len(feature.input_ids), 1)
            )
            data["constituent"].x = resu
            data["constituent"].y = y

            data["constituent", "connect", "constituent"] = torch.tensor(
                r_c
            ).contiguous()
            data["constituent", "connect", "token"] = torch.tensor(r_t).contiguous()
            graph_f.write(
                jsonpickle.encode({"qid": graph_id, "graph": data}, indent=None) + "\n"
            )
            graph_ids += [graph_id]
            graph_id += 1
