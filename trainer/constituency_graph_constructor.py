...
class ConstituencyParser:
    def __init__(self):
        self.pipeline = stanza.Pipeline(
            lang="en", processors="tokenize,pos,constituency"
        )
...
class GraphConstructor(object):
    def __init__(self, dataset_name: str, dataset_split: str) -> None:
        ...
        assert self.dataset_name == "squad" and self.dataset_split_name in [
            "train",
            "validation",
        ]
    ...
    def virtualize(self, index, graph_data):
        G = torch_geometric.utils.to_networkx(
            graph_data.to_homogeneous(), to_undirected=False
        )
    ...
    def dump_instance(self, graph_data):
        with open(
            f'./data/{self.dataset_split_name}/{graph_data[1]["qid"]}.pkl', "wb"
        ) as dump_file:
...
class ConstituencyGraphConstructor(GraphConstructor):
    def __init__(
        ...
        dataset_name: str,
        dataset_split: str,
        representation_model: str = "bert-base-uncased",
    ) -> None:
        super(ConstituencyGraphConstructor, self).__init__(dataset_name, dataset_split)
        self.tokenizer = HFTokenizer(representation_model)

        # Constituency Parser
        self.c_parser = ConstituencyParser()

        # MetaData
        self.metadata = (["context"], [("context", "connect", "context")])
    ...
    def construct_context(self, current_node, parent_node, answer_candidates):
        if not current_node.is_leaf():
            ...
            current_representation = self.tokenizer.get_pooled_representation(
                current_span
            )
            current_index = len(self.X[current_label])

            self.X[current_label] += [current_representation]
            self.Y[current_label] += [
                max(
                    [
                        self.distance(current_span, answer)
                        for answer in answer_candidates
                    ]
                )
            ]
            self.T[current_label] += [(current_span, current_representation)]
            self.R[f"{current_label}={parent_node[0]}"] += [
                [current_index, parent_node[1]]
            ]

            for child_node in current_node.children:
                self.construct_context(
                    child_node, [current_label, current_index], answer_candidates
                )
    ...
    def construct(self, data):
        ...
        # Context Graph
        current_reresentation = self.tokenizer.get_pooled_representation_str(context)
        ...
        for sentence_index, sentence in enumerate(self.c_parser.get_sentences(context)):
            ...
            current_reresentation = self.tokenizer.get_pooled_representation(
                current_span
            )
            self.X["context"] += [current_reresentation]
            self.Y["context"] += [
                max([self.distance(context, answer) for answer in answer_candidates])
            ]
            self.T["context"] += [(current_span, current_reresentation)]
            self.R["context=context"] += [[current_index, 0]]
            self.construct_context(
                sentence.constituency, ("context", current_index), answer_candidates
            )
        ...
        for relation in self.R:
            ...
            graph_data[head, "connect", tail].edge_index = (
                torch.tensor(self.R[relation]).t().contiguous()
            )
