...
from trainer.constituency_graph_constructor import ConstituencyGraphConstructor
...
class GraphQADataset:
    ...
    def transform(self):
        # TODO: Test with the following transformations
...
class SquadDataset(Dataset):
    ...
    def build(self):
        ...
        for instance in tqdm(list(self.dataset)[self.size_from : self.size]):
            ...
            candidates = [[" ".join(pc[0]), pc[1], 1] for pc in positive_candidates] + [
                [" ".join(nc[0]), nc[1], 0]
                for nc in sample(negative_candidates, len(positive_candidates))
            ]

            for answer_index, answer in enumerate(answers["text"]):
                for can_index, c in enumerate(candidates):
                    processed_input = self.tokenizer(
                        f"[CLS] {question} [SEP] {answer} [SEP] {context}",
                        max_length=512,
                        padding="max_length",
                        truncation=True,
                        return_tensors="pt",
                    ...
                    instance = {
                        ...
                        "input_attention_mask": processed_input[
                            "attention_mask"
                        ].squeeze(),
